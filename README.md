...
- **작업 옵션 분리**: NFD 파일명 변환과 하위 폴더에 흩어진 파일 모으기를 체크박스로 각각 켜고 끌 수 있습니다.
- **흩어진 파일 모으기**: 선택한 폴더의 하위 폴더에 있는 파일을 선택 폴더로 이동합니다. 같은 이름이 이미 있으면 ` (1)`, ` (2)` 형식의 번호를 붙여 덮어쓰기를 방지합니다.
...
3. 파일이나 폴더를 열려있는 프로그램 창으로 드래그 앤 드롭합니다.
4. `작업 옵션`에서 `NFD 파일명을 NFC로 변환`, `하위 폴더의 흩어진 파일을 선택 폴더로 모으기` 중 필요한 기능을 선택한 뒤 실행합니다.
